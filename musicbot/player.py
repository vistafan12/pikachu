...
class MusicPlayer(EventEmitter):
    ...
    def resume(self):
        ...
        raise ValueError('Nie mogę wrócić z playbacku %s' % self.state)
    ...
    def pause(self):
        ...
        raise ValueError('Nie mogę zatrzymać playera w %s' % self.state)
    ...
    def _playback_finished(self):
        ...
        if not self.bot.config.save_videos and entry:
            if any([entry.filename == e.filename for e in self.playlist.entries]):
                print("[Config:SaveVideos] Przewijam, znaleziono piosenkę w queue.")
    ...
    async def websocket_check(self):
        ...
        while not self.is_dead:
            try:
                ...
                if self.bot.config.debug_mode:
                    print("[Debug] Głosowy websocked jest włączony: %s, odświerzam" % self.voice_client.ws.state_name)
