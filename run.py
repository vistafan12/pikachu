...
class PIP(object):
    ...
    def run(cls, command, check_output=False):
        if not cls.works():
            raise RuntimeError("Nie można zinportować pip.")
    ...
    def get_module_version(cls, mod):
        try:
            ...
            if expectedversion.startswith('Version: '):
                ...
                return [x.split()[1] for x in datas if x.startswith("Wersja: ")][0]
        except:
            pass
...
def main():
    if not sys.version_info >= (3, 5):
        print("Python 3.5+ jest potrzbeny. Twoja wersja to: %s" % sys.version.split()[0])
        print("Szukam lokalizacji Pythona 3.5...")
        ...
        if sys.platform.startswith('win'):
            ...
            if pycom:
                print("Python 3 znaleziony.  Uruchamiam bota...")
            ...
            if pycom:
                print("\nPython 3 znaleziony.  Restartuje bota: ")
                print("  %s run.py\n" % pycom)

                os.execlp(pycom, pycom, 'run.py')

        print("Proszę uruchomić bota w Python 3.5")
    ...
    while tryagain:
        ...
        if sleeptime:
            print("Restartuje w {} sekundach...".format(loops*2))
